...
Class Vt_return_material extends CI_Model
{
	var $table = 't_wh';
	var $table_id = 'id_wh';
	...
	function __construct()
	{
		...
		$this->basesql = "select ta.*, tb.nama_jenis_mutasi, ifnull(tc.detail_count, 0) detail_count "
                        . " from $this->table ta "
                        . "     LEFT JOIN m_jenis_mutasi tb ON tb.id_jenis_mutasi = ta.id_jenis_mutasi "
                        . "     left join (select id_wh, count(*) detail_count from t_wh_detail where deleted_at is null and id_koordinat='7' group by id_wh) tc on tc.id_wh = ta.id_wh "
                        . " where ta.deleted_at is null AND ta.id_jenis_mutasi IN (12)";
	}
		...
		if($num>0){
			...
			foreach ($res->result() as $r){
				...
				if($opt){
					...
					$r->option .= "<a href='".base_url('warehouse/return_material/print/'.$r->id_wh)."' class='btn btn-xs btn-default'><i class='fal fa-print'></i></a>";
				}
				$data[] = $r;
				$i++;
			}
	...
	function delete($id)
	{
		...
		$b->id_user_deleted = $this->session->userdata('id_user');
