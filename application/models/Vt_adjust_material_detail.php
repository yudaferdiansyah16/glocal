...
Class Vt_adjust_material_detail extends CI_Model
{
	...
	function __construct()
	{
		...
		$this->basesql = "SELECT ta.*, tb.qty_dn, tb.no_sj, tb.tanggal_sj, tb.harga, tb.seri_barang, tc.id_supplier, tc.id_fasilitas, tc.kode_dn, tc.no_invoice, tc.tgl_kedatangan, tc.ID_HEADER as id_header, te.kode_barang, te.nama_barang, tf.KODE_SATUAN kode_satuan_terkecil, tf.URAIAN_SATUAN uraian_satuan_terkecil, tg.KODE_SATUAN kode_satuan_terbesar, tg.URAIAN_SATUAN uraian_satuan_terbesar, tj.kode_po, tj.tanggal_dibuat, tk.kode_valuta, tk.uraian_valuta, tl.id_akun, tm.id_jenis_mutasi, tm.kode_mutasi, tm.tanggal_terima, tn.NAMA as nama_supplier, tr.no_job, tr.tanggal_job, tt.nama_gudang, ts.nama_koordinat, ifnull(tu.qty_stock, 0) as qty_stock FROM t_wh_detail ta LEFT JOIN t_detail_dn tb ON tb.id_detail_dn = ta.id_detail_dn LEFT JOIN t_dn tc ON tc.id_dn = tb.id_dn LEFT JOIN m_fasilitas td ON td.id_fasilitas = tc.id_fasilitas LEFT JOIN m_sub_barang AS te ON te.id_sub_barang = ta.id_sub_barang LEFT JOIN ".getdbtpb($this).".referensi_satuan tf ON tf.ID = ta.id_satuan_terkecil LEFT JOIN ".getdbtpb($this).".referensi_satuan tg ON tg.ID = ta.id_satuan_terbesar LEFT JOIN m_gudang th ON th.id_gudang = ta.id_gudang LEFT JOIN t_detail_po ti on ti.id_detail_po = tb.id_detail_po LEFT join t_po tj on tj.id_po = ti.id_po left join ".getdbtpb($this).".referensi_valuta as tk on tk.ID = tj.id_valuta LEFT JOIN m_class tl on tl.id_class = te.id_class LEFT JOIN t_wh tm on tm.id_wh = ta.id_wh left JOIN ".getdbtpb($this).".referensi_pengusaha tn ON tn.ID = tc.id_supplier LEFT JOIN t_detail_pp tp ON tp.id_detail_pp = ti.id_detail_pp LEFT JOIN t_detail_job tq ON tq.id_detail_job = tp.id_detail_job LEFT JOIN t_job tr ON tr.id_job = tq.id_job LEFT JOIN m_koordinat ts on ts.id_koordinat = ta.id_koordinat LEFT JOIN m_gudang tt ON tt.id_gudang = ts.id_gudang LEFT JOIN ($sqlstock) tu on tu.id_detail_dn = ta.id_detail_dn and tu.id_job = ta.id_job and tu.id_production = ta.id_production and tu.id_koordinat = ta.id_koordinat and tu.rate = ta.rate and tu.id_sub_barang = ta.id_sub_barang and tu.id_satuan = ta.id_satuan_terkecil WHERE ta.deleted_at IS NULL and tm.id_jenis_mutasi IN (15) ";
	}
	...
	function viewDT($in, $opt = true)
	{
		...
		if($num>0){
			...
			foreach ($res->result() as $r){
				...
				$r->option .= "<a href='".base_url('warehouse/adjust_material/print/'.$r->id_wh)."' class='btn btn-xs btn-default'><i class='fal fa-print'></i></a>";
				$data[] = $r;
				$i++;
