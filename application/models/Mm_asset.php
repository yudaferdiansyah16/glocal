...
Class Mm_asset extends CI_Model
{
	var $table = 'm_asset';
	...
	function __construct()
	{
		...
		$this->basesql = "select ta.*, tb.nama_barang as nama_barang_parent, tc.URAIAN_SATUAN as nama_satuan_terkecil, td.URAIAN_SATUAN as nama_satuan_terbesar, te.nama_kategori, tf.nama_class, tg.URAIAN_ASAL_BARANG as nama_asal, th.nama_brand from $this->table ta inner join m_sub_barang tb on ta.id_barang = tb.id_sub_barang inner join ".getdbtpb($this).".referensi_satuan tc on ta.id_satuan_terkecil = tc.ID inner join ".getdbtpb($this).".referensi_satuan td on ta.id_satuan_terbesar = td.ID inner join m_kategori te on ta.id_kategori = te.id_kategori inner join m_class tf on ta.id_class = tf.id_class inner join ".getdbtpb($this).".referensi_asal_barang tg on ta.id_asal = tg.ID inner join m_brand th on ta.id_brand = th.id_brand where ta.deleted_at is null";
	}
