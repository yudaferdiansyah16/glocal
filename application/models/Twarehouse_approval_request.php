...
Class Twarehouse_approval_request extends CI_Model
{
	var $table = 't_wh';
	var $table_id = 'id_wh';
	...
	function __construct()
	{
		...
		$this->basesql = "select ta.* from $this->table ta where id_jenis_mutasi = '13' and ta.approval_1 = '1'";
	}
	...
	function viewDTPacking($in, $opt = true)
	{
		$start = $in->start;
		$sqlmain = $this->basesql;
		$sql = "select ta.* from $this->table ta where id_jenis_mutasi = '14' and ta.approval_1 = '1'";
		$res = $this->db->query($sql);
		$recordsTotal = $res->num_rows();
	...
	function getFg($id)
	{
		$sql = "select *,id_production from (select ta.* from $this->table ta where id_jenis_mutasi = '14' and ta.approval_1 = '1') pa 	LEFT JOIN t_wh_detail tw ON pa.id_wh = tw.id_wh where pa.$this->table_id = '$id'";
		$res = $this->db->query($sql);
		$row = $res->row();
		// printJSON($res);
		return $row;
	...
	{
		$sql = "select * from t_wh ta where ta.kode_mutasi = '$id'";
		$res = $this->db->query($sql);
		$row = $res->row();
		return $row;
	}
