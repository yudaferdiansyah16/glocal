...
Class Mt_main_so_detail extends CI_Model
{
	...
	function __construct()
	{
		...
		$sqlcall = "SELECT tb.id_main_so, id_sub_barang, SUM( qty_po ) qty_po_call FROM t_detail_po ta INNER JOIN t_po tb ON tb.id_po = ta.id_po WHERE tb.type_trans = 'sales' AND id_main_so IS NOT NULL GROUP BY tb.id_main_so, id_sub_barang";
		$this->basesql = "select ta.*, te.kode_po, tb.kode_barang, tb.nama_barang as nama_sub_barang, tc.kode_kemasan, tc.uraian_kemasan, td.kode_satuan, td.uraian_satuan, tb.id_satuan_terkecil, sk.kode_satuan as kode_satuan_terkecil, sk.uraian_satuan as uraian_satuan_terkecil, tb.id_satuan_terbesar, sb.kode_satuan as kode_satuan_terbesar, sb.uraian_satuan as uraian_satuan_terbesar, IFNULL(tg.qty_po_call, 0) as qty_po_call, ta.qty_po - IFNULL(tg.qty_po_call, 0) as qty_sisa_call from $this->table ta left join m_sub_barang as tb on tb.id_sub_barang = ta.id_sub_barang left join ".getdbtpb($this).".referensi_kemasan as tc on tc.id = ta.id_kemasan left join ".getdbtpb($this).".referensi_satuan as td on td.id = ta.id_satuan left join ".getdbtpb($this).".referensi_satuan sk on sk.id = tb.id_satuan_terkecil left join ".getdbtpb($this).".referensi_satuan as sb on sb.id = tb.id_satuan_terbesar inner join t_po as te on te.id_po = ta.id_po inner join m_customer_suplier as tf on tf.id_customer = te.id_supplier left join ($sqlcall) tg ON tg.id_main_so = ta.id_po AND tg.id_sub_barang = ta.id_sub_barang where te.type_trans = 'main_so' and ta.deleted_at is null";
	}
	...
	{
		$start = $in->start;
		$this->db->select("ta.*, tb.kode_barang, tb.nama_barang as nama_sub_barang, tc.kode_kemasan, tb.id_satuan_terkecil, sk.KODE_SATUAN as kode_satuan_kecil, tb.id_satuan_terbesar, sb.KODE_SATUAN as kode_satuan_besar, td.kode_po, td.id_supplier, td.po_buyer, td.tanggal_dibuat, te.nama as nama_supplier, tf.KODE_VALUTA as kode_valuta, tf.URAIAN_VALUTA as nama_valuta, tg.KODE_SATUAN as kode_satuan, tg.URAIAN_SATUAN as nama_satuan");
		$this->db->from("$this->table as ta");
		$this->db->join("m_sub_barang as tb", "tb.id_sub_barang = ta.id_sub_barang", "left");
		$this->db->join(getdbtpb($this).".referensi_kemasan as tc", "tc.id = ta.id_kemasan", "left");
		$this->db->join("t_po as td", "td.id_po = ta.id_po", "left");
		$this->db->join("m_customer_suplier as te", "te.id_customer = td.id_supplier", "left");
		$this->db->join(getdbtpb($this).".referensi_valuta as tf", "tf.id = td.id_valuta", "left");
		$this->db->join(getdbtpb($this).".referensi_satuan as tg", "tg.id = ta.id_satuan", "left");
		$this->db->join(getdbtpb($this).".referensi_satuan as sk", "sk.id = tb.id_satuan_terkecil", "left");
		$this->db->join(getdbtpb($this).".referensi_satuan as sb", "sb.id = tb.id_satuan_terbesar", "left");
	...
	function viewdetaildt($in,$id_po)
	{
		$start = $in->start;
		$sqlmain = "select * from ($this->basesql) pa where id_po='$id_po'";
		$sql = "select * from ($sqlmain) pa";
		$res = $this->db->query($sql);
		$recordsTotal = $res->num_rows();
