...
Class Mm_detail_supplier_destination extends CI_Model
{
	var $table = 'm_detail_supplier_destination';
	var $table_id = 'id_supplier_destination';
	...
	function __construct()
	{
		...
		$this->basesql = "select ta.*,tb.nama as nama_supplier, tc.URAIAN_NEGARA as nama_negara from $this->table ta left join m_customer_suplier tb on ta.id_supplier = tb.id_customer left join ".getdbtpb($this).".referensi_negara tc on ta.id_negara = tc.ID inner join m_status td on ta.id_status = td.id_status where ta.deleted_at is null";
	}
	...
	function viewDT($in, $opt = true)
	{
		...
		if (isset($in->id_supplier)) {
			$sqlmain .= " and ta.id_supplier = '$in->id_supplier' ";
		}
