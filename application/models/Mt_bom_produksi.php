...
Class Mt_bom_produksi extends CI_Model
{
	var $table = 't_bom';
    var $table_id = 'id_bom';
    ...
    function __construct()
    {
        ...
        $this->basesql = "select ta.*, tb.nama as nama_supplier, tc.id_po, tc.kode_po, tc.po_buyer, tc.tanggal_dibuat, tc.id_supplier, td.status_trans, te.kode_barang, te.nama_barang, te.size, tf.KODE_SATUAN as kode_satuan, tf.URAIAN_SATUAN as uraian_satuan, IFNULL(bw.workflow_count, 0) as workflow_count, IFNULL(bd.detail_count, 0) as detail_count, be.kode_bom kode_bom_master, be.qty as qty_bom_master from $this->table as ta left join t_detail_po tj on tj.id_detail_po =ta.id_detail_po left join t_po as tc on tc.id_po = tj.id_po left join m_customer_suplier as tb on tb.id_customer = tc.id_supplier left join m_status as td on td.id_status = ta.id_status left join m_sub_barang as te on te.id_sub_barang = ta.id_sub_barang left join ".getdbtpb($this).".referensi_satuan as tf on tf.id = ta.id_satuan left join (select id_bom, count(*) as workflow_count from t_bom_workflow group by id_bom) as bw on bw.id_bom = ta.id_bom left join (select t_bom_workflow.id_bom, count(*) as detail_count from t_bom_detail inner join t_bom_workflow on t_bom_workflow.id_bom_workflow = t_bom_detail.id_bom_workflow group by t_bom_workflow.id_bom) as bd on bd.id_bom = ta.id_bom left join t_bom be on be.id_bom = ta.id_bom_master where ta.deleted_at is null and ta.tipe_bom = 'produksi'";
    }
    ...
    function generateCode($bom_date)
	{
		...
		$res = $this->db->query("select kode_bom from $this->table where tanggal_bom >= '".date('Y-m-01', strtotime($bom_date))."' and tanggal_bom <= '".date('Y-m-t', strtotime($bom_date))."' and deleted_at is null and tipe_bom = 'produksi' order by kode_bom desc limit 1");
		$num = $res->num_rows();
		$latest_number = 1;
		if ($num > 0) {
			$index_number = '0000';
	...
	{
		$sql = $this->basesql;
		$sql .= " and (ta.kode_bom LIKE '%".$in->term."%' or te.kode_barang LIKE '%".$in->term."%' or te.nama_barang LIKE '%".$in->term."%' or tc.po_buyer LIKE '%".$in->term."%' or tb.nama LIKE '%".$in->term."%') ";
		$res = $this->db->query($sql);
