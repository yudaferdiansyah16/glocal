...
Class Mreferensi_kemasan extends CI_Model
{
    ...
    {
        parent::__construct();
        $this->table = getdbtpb($this).'.referensi_kemasan';
        $this->table_id = 'ID';
    ...
    function select2($in)
    {
        if(isset($in->term)){
            ...
            $sql = $this->basesql." where (LOWER(KODE_KEMASAN) LIKE '%$q%' OR LOWER(URAIAN_KEMASAN) LIKE '%$q%') ORDER BY URAIAN_KEMASAN";
        ...
        foreach ($res->result() as $r){
            $data[] = array(
                'ID' => $r->ID,
                'KODE_KEMASAN' => $r->KODE_KEMASAN,
                'URAIAN_KEMASAN' => $r->URAIAN_KEMASAN
            );
        }
