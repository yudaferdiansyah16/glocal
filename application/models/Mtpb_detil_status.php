...
Class Mtpb_detil_status extends CI_Model
{
	...
	function __construct()
	{
		...
		$this->table = getdbtpb($this).'.tpb_detil_status';
		$this->table_id = 'ID';
	...
	function approval1($id)
	{
		...
		$this->db->update($this->table, $b);
		// sendTPB($this,'tpb_header','insert',$id);

		// $sql = "select * from ".getdbtpb($this).".tpb_barang where ID_HEADER = $id";
		// $res = $this->db->query($sql);
		// foreach ($res->result() as $r){
		// 	sendTPB($this,'tpb_barang','insert',$r->ID);
		// }

		// $sql = "select * from ".getdbtpb($this).".tpb_dokumen where ID_HEADER = $id";
		// $res = $this->db->query($sql);
		// foreach ($res->result() as $r){
		// 	sendTPB($this,'tpb_barang','insert',$r->ID);
		// }

		// $sql = "select * from ".getdbtpb($this).".tpb_kemasan where ID_HEADER = $id";
		// $res = $this->db->query($sql);
		// foreach ($res->result() as $r){
		// 	sendTPB($this,'tpb_barang','insert',$r->ID);
		// }

		// $sql = "select * from ".getdbtpb($this).".tpb_detail_status where ID_HEADER = $id";
		// $res = $this->db->query($sql);
		// foreach ($res->result() as $r){
		// 	sendTPB($this,'tpb_barang','insert',$r->ID);
		// }
