...
Class Twarehouse_detail_approval_request extends CI_Model
{
	...
	function __construct()
	{
		...
		$this->basesql = "select ta.* from $this->table ta where id_jenis_mutasi = '13' and approval_1 = '1'";
	}
	...
	function viewBasedID($id)
	{
		$in = a2o($this->in);
		$start = $in->start;
		$sqlmain = "select tb.no_job,th.tanggal_terima,th.kode_mutasi, tc.kode_barang, tc.nama_barang, tc.unit_konversi, td.nomor_aju, td.nomor_daftar, td.no_sj ,ta.*,ta.id_satuan_terkecil id_satuan,te.KODE_SATUAN,te.URAIAN_SATUAN  from (select * from t_wh_detail where id_wh = '$id') ta INNER JOIN t_wh  th on  th.id_wh = ta.id_wh  inner join t_job tb on ta.id_job = tb.id_job inner join m_sub_barang tc on ta.id_sub_barang = tc.id_sub_barang left join (select xa.id_wh_detail , xb.NOMOR_AJU as nomor_aju, xb.NOMOR_DAFTAR as nomor_daftar, xc.no_sj from t_wh_detail xa left join ".getdbtpb($this).".tpb_header xb on xa.id_header = xb.ID left join t_detail_dn xc on xa.id_detail_dn = xc.id_detail_dn) td on ta.id_wh_detail = td.id_wh_detail LEFT JOIN smartone_tpb_dps1.referensi_satuan te ON te.ID = tc.id_satuan_terkecil where qty > 0";
		$sql = "select * from ($sqlmain) pa";
		$res = $this->db->query($sql);
		$recordsTotal = $res->num_rows();
