...
Class Mpeb_tblpebdok extends CI_Model
{
	var $table = 'smartone_peb.tblpebdok';
	var $table_id = 'CAR';
	...
	{
		$sql = "select tb.Uraian as uraianDokumen,ta.* from (select * from smartone_peb.tblpebdok where CAR = '$id') ta left join (select * from smartone_peb.tbltabel WHERE KDTAB = '06') tb on ta.KdDok = tb.KDREC";
		$res = $this->db->query($sql);
