	...
	function create($in)
    {
		...
		for($i=0; $i<count($in->t_bayar_detail); $i++){
			if($in->t_bayar_detail[$i]){
				...
				$c->id_akun = $in->hakun[$i];
				$c->trans_description = $in->hinvoice;
				$c->description = $in->hinvoice;
				...
				$d->trans_description = $in->hinvoice;
				$d->description = $in->hinvoice;
