...
Class Twarehouse_detail_request extends CI_Model
{
	...
	function viewReq($id)
	{
		$sql = "select ta.*, tb.kode_barang, tb.nama_barang from (select * from t_request_detail where id_request = '$id') ta inner join m_sub_barang tb on ta.id_sub_barang = tb.id_sub_barang";
		$res = $this->db->query($sql);
	...
	function viewRlz($id)
	{
		$sql = "select ta.*, tc.qty_rlz, td.qty_rtn, (ta.qty_req-ifnull(tc.qty_rlz,0)-ifnull(td.qty_rtn,0)) qty_sisa, tb.nama_barang, tb.kode_barang, tb.kode_satuan_terkecil as kode_satuan from (select xa.id_request, xa.id_detail_dn, xa.id_job, xa.id_master, xa.kode_mutasi, xa.tanggal_mutasi, xa.status, xa.approval_1, xa.approval_2, xa.id_user_approval_1, xa.id_user_approval_2, xa.date_approval_1, xa.date_approval_2, xa.date_closing, xa.id_user_closing, xa.flag_closing, xa.flag_edit, xa.flag_btl, xa.id_user_btl, xa.btl_date, xa.deleted_at, xa.id_jenis_mutasi, xb.qty as qty_req, xb.id_sub_barang from t_request xa inner join t_request_detail xb on xa.id_request = xb.id_request where xa.id_jenis_mutasi = '13' and xa.id_request = '$id') ta inner join v_sub_barang tb on ta.id_sub_barang = tb.id_sub_barang left join (select xa.id_request, xa.id_master, xa.id_jenis_mutasi, sum(xb.qty) as qty_rlz, xb.id_sub_barang from t_request xa inner join t_request_detail xb on xa.id_request = xb.id_request where xa.id_jenis_mutasi = '14' group by xb.id_sub_barang, xa.id_master) tc on (ta.id_request = tc.id_master and ta.id_sub_barang = tc.id_sub_barang) left join (select xa.id_request, xa.id_master, xa.id_jenis_mutasi, sum(xb.qty) as qty_rtn, xb.id_sub_barang from t_request xa inner join t_request_detail xb on xa.id_request = xb.id_request where xa.id_jenis_mutasi = '12' group by xb.id_sub_barang, xa.id_master) td on (ta.id_request = td.id_master and ta.id_sub_barang = td.id_sub_barang) having qty_sisa > 0";
		$res = $this->db->query($sql);
	...
	function viewPrint()
	{
		$sqlmain = "select tb.no_job, tc.kode_barang, tc.nama_barang, td.nomor_aju, td.nomor_daftar, td.no_sj ,ta.* from (select * from t_request_detail where id_request = '$id') ta inner join t_job tb on ta.id_job = tb.id_job inner join m_sub_barang tc on ta.id_sub_barang = tc.id_sub_barang left join (select xa.id_wh_detail , xb.NOMOR_AJU as nomor_aju, xb.NOMOR_DAFTAR as nomor_daftar, xc.no_sj from t_wh_detail xa left join ".getdbtpb($this).".tpb_header xb on xa.id_header = xb.ID left join t_detail_dn xc on xa.id_detail_dn = xc.id_detail_dn) td on ta.id_wh_detail = td.id_wh_detail where qty > 0";
	...
	function viewBasedID($id)
	{
		$in = a2o($this->in);
		$start = $in->start;
		$sqlmain = "select tb.no_job, te.tgl_request, tc.kode_barang, tc.nama_barang, td.nomor_aju, td.nomor_daftar, td.no_sj ,ta.* from (select * from t_request_detail where id_request = '$id') ta inner join t_request te on te.id_request = ta.id_request inner join t_job tb on te.id_job = tb.id_job inner join m_sub_barang tc on ta.id_sub_barang = tc.id_sub_barang left join (select xa.id_wh_detail , xb.NOMOR_AJU as nomor_aju, xb.NOMOR_DAFTAR as nomor_daftar, xc.no_sj from t_wh_detail xa left join ".getdbtpb($this).".tpb_header xb on xa.id_header = xb.ID left join t_detail_dn xc on xa.id_detail_dn = xc.id_detail_dn) td on ta.id_wh_detail = td.id_wh_detail where qty_request > 0";
