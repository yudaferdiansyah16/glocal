...
Class Vt_adjust_material extends CI_Model
{
	var $table = 't_wh';
	var $table_id = 'id_wh';
	...
	function __construct()
	{
		...
		$this->basesql = "select ta.*, tb.nama_jenis_mutasi, ifnull(tc.detail_count, 0) detail_count from $this->table ta LEFT JOIN m_jenis_mutasi tb ON tb.id_jenis_mutasi = ta.id_jenis_mutasi left join (select id_wh, count(*) detail_count from t_wh_detail where deleted_at is null group by id_wh) tc on tc.id_wh = ta.id_wh where ta.deleted_at is null AND ta.id_jenis_mutasi IN (15)";
	}
