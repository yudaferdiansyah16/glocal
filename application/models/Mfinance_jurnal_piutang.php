...
class Mfinance_jurnal_piutang extends CI_Model
{
    var $nowdt, $nowd, $nowt;
    ...
    function create($in)
    {
        for ($x = 0; $x < count($in->id_invoice); $x++) {
            $delimit = explode('==', $in->id_invoice[$x]);
            $count = $delimit[5];
            $nama_supplier = $delimit[1];
            $tgl_trans = $this->nowd;
            // $tgl_trans = reverseDate($in->tanggal_invoice[$count]);
            $no_trans = $this->mt_finance->generateCode($tgl_trans, 'JP');
            $rate = $in->rate[$count];
            ...
            for ($y = 0; $y < count($limit); $y++) {
            	...
            	$items_detail = array(
	                ...
	                'id_akun' => $id_akun,
	                'description' => 'POSTING JURNAL PIUTANG ' . $nama_supplier . ' ('  . $tgl_trans . ')',
            ...
            $this->db->where('id_invoice', $delimit[0]);
            $this->db->update('t_invoice', $items_invoice);
        }
        return true;
    }
    ...
    function create2($in)
    {
        for ($x = 0; $x < count($in->id_invoice); $x++) {
            ...
            $items_detail = array(
                ...
                'description' => $delimit[1] . ' PENJUALAN ' . $nama_supplier . ' ('  . $tgl_trans . ')',
                'jumlah_rp' => $delimit[7],
