...
Class Mm_gudang extends CI_Model
{
    var $table = 'm_gudang';
    var $table_id = 'id_gudang';
    ...
    function __construct()
    {
        ...
        $this->basesql = "select ta.*, tb.status_trans from $this->table as ta inner join m_status as tb on tb.id_status = ta.id_status where ta.deleted_at is null";
    }
        ...
        if($num>0){
            ...
            foreach ($res->result() as $r){
                ...
                if($opt){
					...
					$r->option .= '<a href="'.base_url($this->d->_controller.'/'.$this->d->_method.'/koordinat/'.$r->{$this->table_id}).'" class="btn btn-xs btn-warning"><i class="fal fa fa-list"></i></a> ';
					if ($r->id_gudang != '4') {
