<main id="js-page-content" role="main" class="page-content">
	<ol class="breadcrumb page-breadcrumb">
		<li class="breadcrumb-item"><a href="javascript:void(0);">DPS</a></li>
		<li class="breadcrumb-item">Warehouse</li>
		<li class="breadcrumb-item ">Packing List</li>
		<li class="breadcrumb-item active">Print</li>
		<li class="position-absolute pos-top pos-right d-none d-sm-block"><span class="js-get-date"></span></li>
	</ol>
	<div class="subheader">
	</div>
	<div class="container">
		<div data-size="A4">
			<!--HEADER-->
			<div class="row">
				<div class="col-sm-1">
					<div class="img-container mb-3" align="center">
						<img src="<?= assets_url('img/' . $app->img) ?>" alt="DPS" aria-roledescription="logo" width="60">
					</div>
				</div>
				<div class="col-sm-10">
					<div class="d-flex align-items-center">
						<h2 class="keep-print-font fw-700 mb-0 text-dark flex-1 position-relative" align="center">
							<?= $app->nama_sbu ?>
						</h2>
					</div>
					<div class="d-flex align-items-center mb-6">
						<h6 class="keep-print-font fw-400 mb-0 text-dark flex-1 position-relative" align="center">
							<?= $app->alamat ?>
						</h6>
					</div>
				</div>
				<div class="col-sm-1 text-right">
					<div id="qrcode"></div>
					<input type="hidden" id="kode_qrcode" value="<?= $t_stuffing->kode_stuffing ?>">
				</div>
			</div>
			<div class="row">
				<div class="col-sm-12 text-center">
					<p class="fw-300 display-4 fw-500 color-primary-600 keep-print-font l-h-n m-0">
						Packing List
					</p>
				</div>
			</div>
			<!--END HEADER-->
			<div class="row">
				<div class="col-sm-12 d-flex align-items-right">
					<dl class="row">
						<dt class="col-sm-6 col-lg-4">Beneficiary</dt>
						<dd class="col-sm-6 col-lg-8">: <?= placeValue($t_stuffing, 'nama_supplier') ?></dd>
						<dt class="col-sm-6 col-lg-4">Applicant</dt>
						<dd class="col-sm-6 col-lg-8">: <?= placeValue($t_stuffing, 'nama_supplier') ?></dd>
					</dl>
					<dl class="row">
						<dt class="col-sm-6 col-lg-4">Packing List Number</dt>
						<dd class="col-sm-6 col-lg-8">: <?= $t_stuffing->kode_stuffing ?></dd>
						<dt class="col-sm-6 col-lg-4">PO Number</dt>
						<dd class="col-sm-6 col-lg-8">: <?= $t_stuffing->kode_stuffing ?></dd>
					</dl>
				</div>
				<div class="col-sm-12 d-flex align-items-right">
					<h6 class="keep-print-font fw-400 mb-0 text-dark flex-1 position-relative" align="left">

					</h6>
				</div>
				<div class="col-sm-4 ml-sm-auto">
					<div class="table-responsive">
						<table class="table table-sm table-clean text-right">
							<tbody>
								<!-- barcode demo only -->
								<!--							<img id="qrcode" alt="" class="position-absolute pos-top pos-right height-3 mt-1 hidden-md-down keep-print" src="data:image/png;base64,">-->
							</tbody>
						</table>
					</div>
				</div>
			</div>
			<div class="row">
				<div class="col-sm-12">
					<div class="table-responsive">
						<table class="table table-mt-5 table-sm" id="" role="grid" style="width: 99%">
							<thead>
								<tr>
									<th class="text-center table-scale-border-bottom table-scale-border-top" rowspan="2">No</th>
									<th class="text-left table-scale-border-bottom table-scale-border-top" rowspan="2">Description Of Good</th>
									<th class="text-left table-scale-border-bottom table-scale-border-top" rowspan="2">Size</th>
									<th class="text-left table-scale-border-bottom table-scale-border-top" rowspan="2">Number Of Carton</th>
									<th class="text-left table-scale-border-bottom table-scale-border-top" rowspan="2">Number Of Sat</th>
									<th class="text-center table-scale-border-bottom table-scale-border-top" colspan="2">Total Of Wight</th>
								</tr>
								<tr>
									<th class="text-left table-scale-border-bottom table-scale-border-top" rowspan="2">Net</th>
									<th class="text-left table-scale-border-bottom table-scale-border-top" rowspan="2">Gross</th>
								</tr>
							</thead>
							<tbody>
								<?php $totalnetto = $totalbruto = 0;
								foreach ($t_detail_stuffing as $i => $detail) : ?>
									<tr>
										<td class="text-center"><?= $i + 1 ?></td>
										<td><?= $detail->nama_barang ?><br>
											<small><?= $detail->kode_barang ?></small></td>
										<td><?= $detail->kode_po ?><br>
											<small>SO Date: <?= date('d-m-Y', strtotime($detail->tanggal_dibuat)) ?></small></td>
										<td><?= $detail->kode_satuan ?></td>
										<td><?= $detail->kode_satuan ?></td>
										<td><?= number_format($detail->netto, 0) ?></td>
										<td><?= number_format($detail->bruto, 0) ?></td>
									</tr>
									<?php $totalnetto += $detail->netto;
									$totalbruto += $detail->bruto; ?>
								<?php
								endforeach;
								?>
							</tbody>
							<tfoot>
								<tr>
									<th class="text-center table-scale-border-top table-scale-border-bottom" colspan="2">Total</th>
									<th class="text-right table-scale-border-top table-scale-border-bottom"></th>
									<th class="text-center table-scale-border-top table-scale-border-bottom"></th>
									<th class="text-right table-scale-border-top table-scale-border-bottom"></th>
									<th class="text-center table-scale-border-top table-scale-border-bottom"><?= $totalnetto ?></th>
									<th class="text-center table-scale-border-top table-scale-border-bottom"><?= $totalbruto ?></th>
								</tr>
							</tfoot>
						</table>
					</div>
				</div>
				<div class="col-sm-12">
					<dl class="row">
						<dt class="col-sm-12 col-lg-12 text-right">Gresik, <?php $date = date('d-m-Y');
																			echo $date; ?></dt>
						<dt class="col-sm-12 col-lg-12 text-right"><?= $app->nama_sbu ?></dt>
						<dt class=" col-sm-12 col-lg-12 text-right">&nbsp;</dt>
						<dt class="col-sm-12 col-lg-12 text-right">&nbsp;</dt>
						<dt class=" col-sm-12 col-lg-12 text-right">&nbsp;</dt>
						<dt class="col-sm-12 col-lg-12 text-right">PIMPINAN</dt>
					</dl>
				</div>

				<div class=" col-sm-12">
					<dl class="row">
						<dt class="col-sm-2"><b>Container No</b></dt>
						<dd class="col-sm-10"><b>: <?= $t_stuffing->container_number ?></b> </dd>
						<dt class="col-sm-2"><b>Seal No</b></dt>
						<dd class="col-sm-10"><b>: <?= $t_stuffing->seal_number ?></b> </dd>
					</dl>
				</div>
			</div>
		</div>
	</div>
</main>

<nav class="shortcut-menu d-none d-sm-block">
	<input type="checkbox" class="menu-open" name="menu-open" id="menu_open" />
	<label for="menu_open" class="menu-open-button ">
		<span class="app-shortcut-icon d-block"></span>
	</label>
	<a href="#" class="menu-item btn" data-toggle="tooltip" data-placement="left" title="Scroll Top">
		<i class="fal fa-arrow-up"></i>
	</a>
	<a href="#" class="menu-item btn" data-action="app-fullscreen" data-toggle="tooltip" data-placement="left" title="Full Screen">
		<i class="fal fa-expand"></i>
	</a>
	<a href="#" class="menu-item btn" data-action="app-print" data-toggle="tooltip" data-placement="left" title="Print page">
		<i class="fal fa-print"></i>
	</a>
</nav>
<script>
	let idpp = <?= $idpp ?>;
</script>