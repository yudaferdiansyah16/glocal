				...
				<div class="card-header">
					...
					<?php if($t_po->approval_1 == 1 && $t_po->approval_2 == 0): ?>
						<button type="button" disabled class="btn btn-success btn-sm btn-pills waves-effect waves-themed">
							<i class="fa fal fa-check"></i> Approved
						</button>
					<?php endif; ?>
					<?php if($t_po->approval_1 == 1 && $t_po->approval_2 == 1): ?>
						<button type="button" disabled class="btn btn-primary btn-sm btn-pills waves-effect waves-themed">
							<i class="fa fal fa-check"></i> Released
						</button>
					<?php endif; ?>
				</div>
				<div class="card-body">
					<dl class="row">
						...
						<dd class="col-sm-9"><?= $t_po->nama_supplier ?></dd>
					...
					<div class="row">
						<div class="col-md-12">
							<?php if($t_po->approval_2 == 0): ?>
								<form class="form-inline" style="float:left; margin-left: 5px;" method="post" action="<?= base_url('sales/main_so/approve') ?>">
									...
									<input type="hidden" name="approval_1" value="<?=($t_po->approval_1 == 0 ? 1 : 0)?>">
										<?php if($t_po->approval_1 == 0): ?>
											<button type="submit" class="btn btn-sm btn-success"><i class="fal fa fa-check-circle"></i> Approve
										</button>
										<?php else: ?>
											<button type="submit" class="btn btn-sm btn-secondary"><i class="fal fa fa-circle"></i> Disapprove
											</button>
										<?php endif; ?>
								</form>
							<?php endif; ?>
							<?php if($t_po->approval_1 == 1): ?>
								<form class="form-inline" style="float:left; margin-left: 5px;" method="post" action="<?= base_url('sales/main_so/approve') ?>">
									...
									<?php if($t_po->approval_2 == 0): ?>
										<button type="submit" class="btn btn-sm btn-primary"><i class="fal fa fa-check-circle"></i> Release
										</button>
									<?php else: ?>
										<button type="submit" class="btn btn-sm btn-secondary"><i class="fal fa fa-circle"></i> Unrelease
										</button>
									<?php endif; ?>
								</form>
							<?php endif; ?>
