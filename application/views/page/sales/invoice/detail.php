			...
			<div class="card mb-g">
				...
				<div class="card-body">
					<dl class="row">
						...
						<!-- <dt class="col-sm-2">Type of Mutation</dt>
						<dd class="col-sm-9"><?= $t_invoice->nama_jenis_mutasi ?></dd> -->
						<?php if (!empty($t_invoice->id_supplier)): ?>
						<dt class="col-sm-2">Supplier</dt>
						<dd class="col-sm-9"><?= $t_invoice->nama_supplier ?></dd>
						<?php endif; ?>
						<?php if (!empty($t_invoice->id_customer)): ?>
						<dt class="col-sm-2">Customer</dt>
						<dd class="col-sm-9"><?= $t_invoice->nama_customer ?></dd>
						<?php endif; ?>
						<dt class="col-sm-2">Destination</dt>
						<dd class="col-sm-9"><?= $t_invoice->alamat ?>, <?= $t_invoice->uraian_negara ?></dd>
						<dt class="col-sm-2">BL Number</dt>
						<dd class="col-sm-9"><?= !empty($t_invoice->no_bl) ? $t_invoice->no_bl : '-' ?></dd>
								...
								<table class="table table-striped table-bordered table-hover table-sm">
									...
									<?php foreach ($t_invoice_detail as $i => $detail): ?>
										<tr>
											...
											<td>
												<?=$detail->kode_stuffing?><br>
												<small><?= date('d-m-Y', strtotime($detail->tanggal_stuffing)) ?></small>
											</td>
											<td class="text-right">
						...
						<div class="row">
							<div class="col-md-12">
								<?php if ($t_invoice->approval_1 == 0) { ?>
									<a href="<?=base_url($_controller.'/'.$_method.'/approval1/'.$t_invoice->id_invoice)?>" class="btn btn-sm btn-success"><i class="fal fa fa-check-circle"></i> Approve 1</a>
								<?php }?>
								<?php if ($t_invoice->approval_1 == 1 && $t_invoice->approval_2 == 0) { ?>
									<a href="<?=base_url($_controller.'/'.$_method.'/unapprove1/'.$t_invoice->id_invoice)?>" class="btn btn-sm btn-secondary"><i class="fal fa fa-circle"></i> Disapprove 1</a>
									<a href="<?=base_url($_controller.'/'.$_method.'/approval2/'.$t_invoice->id_invoice)?>" class="btn btn-sm btn-primary"><i class="fal fa fa-check-circle"></i> Approve 2</a>
								<?php }?>
								<?php if ($t_invoice->approval_2 == 1) { ?>
									<a href="<?=base_url($_controller.'/'.$_method.'/unapprove2/'.$t_invoice->id_invoice)?>" class="btn btn-sm btn-secondary"><i class="fal fa fa-circle"></i> Disapprove 2</a>
								<?php }?>
