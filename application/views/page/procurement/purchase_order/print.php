<main id="js-page-content" role="main" class="page-content">
	<ol class="breadcrumb page-breadcrumb">
		<li class="breadcrumb-item"><a href="javascript:void(0);">GCI</a></li>
		<li class="breadcrumb-item">Procurement</li>
		<li class="breadcrumb-item ">Purchase Order</li>
		<li class="breadcrumb-item active">Print</li>
		<li class="position-absolute pos-top pos-right d-none d-sm-block"><span class="js-get-date"></span></li>
	</ol>
	<div class="subheader">
	</div>
	<div class="container">
		<div data-size="A4">
			<!--HEADER-->
			<div class="row">
				<div class="col-sm-2">
					<div class="img-container mb-3" align="left">
						<img src="<?= assets_url('img/'.$app->img) ?>" alt="GCI" aria-roledescription="logo" height="60" width="auto">
					</div>
				</div>
				<div class="col-sm-8">
					<div class="d-flex align-items-center">
						<h2 class="keep-print-font fw-700 mb-0 text-dark flex-1 position-relative" align="center">
							<?=$app->nama_sbu?>
						</h2>
					</div>
					<div class="d-flex align-items-center mb-6">
						<h6 class="keep-print-font fw-400 mb-0 text-dark flex-1 position-relative" align="center">
							<?=$app->alamat?>
						</h6>
					</div>
				</div>
				<div class="col-sm-1 text-right">
					<div id="qrcode"></div>
					<input type="hidden" id="kode_qrcode" value="<?=$poheader->kode_po?>">
				</div>
			</div>
			<div class="row">
				<div class="col-sm-6 text-left">
					<p class="fw-300 display-4 fw-500 color-primary-600 keep-print-font l-h-n m-0">
						Purchase Order
					</p>
				</div>
				<div class="col-sm-6 text-right">
					<div class="text-dark fw-700 h1 mb-g keep-print-font">
						<p># <?=$poheader->kode_po?></p>
					</div>
				</div>
			</div>
			<!--END HEADER-->
			<div class="row">
				<div class="col-sm-12 d-flex">
					<dl class="row">
						<dt class="col-sm-6 col-lg-4">Supplier</dt>
						<dd class="col-sm-6 col-lg-8">: <?=placeValue($poheader,'nama_supplier')?></dd>
						<dt class="col-sm-6 col-lg-4">Date</dt>
						<dd class="col-sm-6 col-lg-8">: <?= date('F d, Y', strtotime($poheader->tanggal_dibuat))?></dd>
					</dl>
				</div>
				<div class="col-sm-4 ml-sm-auto">
					<div class="table-responsive">
						<table class="table table-sm table-clean text-right">
							<tbody>
							<!-- barcode demo only -->
<!--							<img id="qrcode" alt="" class="position-absolute pos-top pos-right height-3 mt-1 hidden-md-down keep-print" src="data:image/png;base64,">-->
							</tbody>
						</table>
					</div>
				</div>
			</div>
			<div class="row">
				<div class="col-sm-12">
					<div class="table-responsive">
						<table class="table table-mt-5 table-sm" id="dt" role="grid" style="width: 99%">
							<thead>
							<tr>
								<th class="text-center table-scale-border-bottom table-scale-border-top">No</th>
								<th class="text-left table-scale-border-bottom table-scale-border-top">PO</th>
								<th class="text-left table-scale-border-bottom table-scale-border-top">PP</th>
								<th class="text-left table-scale-border-bottom table-scale-border-top">Job</th>
								<th class="text-right table-scale-border-bottom table-scale-border-top">Barang</th>
								<th class="text-center table-scale-border-bottom table-scale-border-top">Jumlah</th>
								<th class="text-center table-scale-border-bottom table-scale-border-top">Harga</th>
								<th class="text-center table-scale-border-bottom table-scale-border-top">Keterangan</th>
							</tr>
							</thead>
							<tbody>
							
							<?php
							$totalharga = $totalqty = 0;
							$i = 0;
							?>
							<?php foreach ($podetail as $detail): ?>
								<tr>
									<td class="text-center"><?= $i + 1 ?></td>
									<td class="text-left"><?= $poheader->kode_po?></td>
									<td class="text-left"><?= $detail->kode_bom?></td>
									<td class="text-left"><?= $detail->no_job?></td>
									<td class="text-right"><?= $detail->barang ?></td>
									<td class="text-right"><?= $detail->qty_po ?></td>
									<td class="text-right"><?= $detail->harga ?></td>
									<td class="text-right"><?= $detail->keterangan ?></td>
								</tr>
								<?php 
								$totalqty += floatval($detail->qty);
								$totalharga += floatval(preg_replace('/[^\d.]/', '', $detail->harga));
								 $i++; endforeach; ?> 
							</tbody>
							<tfoot>
								<tr>
									<th class="text-center table-scale-border-top table-scale-border-bottom" colspan="6">Total</th>
									<th class="text-right table-scale-border-top table-scale-border-bottom"><?=number_format($totalharga,2)?></th>
									<th class="text-center table-scale-border-top table-scale-border-bottom"></th>
								</tr>
							</tfoot>
						</table>
					</div>
				</div>
				<div class="col-sm-12">
					<dl class="row">
						<dt class="col-sm-2"><b>Due Date</b></dt>
						<dd class="col-sm-10"><b>:</b> <?= date('F d, Y', strtotime($poheader->tanggal_dibutuhkan))?></dd>
						<dt class="col-sm-2"><b>Remark</b></dt>
						<dd class="col-sm-10"><b>:</b> <?=$poheader->catatan_po?></dd>
					</dl>
				</div>
			</div>
		</div>
	</div>
</main>

<nav class="shortcut-menu d-none d-sm-block">
	<input type="checkbox" class="menu-open" name="menu-open" id="menu_open" />
	<label for="menu_open" class="menu-open-button ">
		<span class="app-shortcut-icon d-block"></span>
	</label>
	<a href="#" class="menu-item btn" data-toggle="tooltip" data-placement="left" title="Scroll Top">
		<i class="fal fa-arrow-up"></i>
	</a>
	<a href="#" class="menu-item btn" data-action="app-fullscreen" data-toggle="tooltip" data-placement="left" title="Full Screen">
		<i class="fal fa-expand"></i>
	</a>
	<a href="#" class="menu-item btn" data-action="app-print" data-toggle="tooltip" data-placement="left" title="Print page">
		<i class="fal fa-print"></i>
	</a>
</nav>
<script>
	let idpp = <?=$idpp?>;
</script>
