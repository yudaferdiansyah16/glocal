			...
			<div class="card mb-g">
				<div class="card-body">
					<form method="post" action="<?=base_url('master/job/detail_update')?>">
						<input type="hidden" name="t_job[id_job]" value="<?=$t_job->id_job?>">
						<input type="hidden" name="t_job[id_bom]" value="<?=$t_job->id_bom?>" class="id_bom">
						...
						<div class="table-responsive">
							<table class="table table-hover table-bordered table-striped table-sm" id="dt" role="grid" style="width: 100%;">
								...
								<tbody>
									<?php foreach($t_detail_job as $i => $r){ ?>
									<tr data-index="<?=$i?>">
										...
										<td>
											<span class="nama_sub_barang"><?=$r->nama_sub_barang?></span><br>
											<small class="kode_barang"><?=$r->kode_barang?></small>
										</td>
										<td class="text-right"><?=($r->qty_bom-$r->qty_total_job+$r->qty_job)?> <?=$r->kode_satuan?></td>
										<td>
											<input type="hidden" class="id_detail_job" name="t_detail_job[<?=$i?>][id_detail_job]" value="<?=$r->id_detail_job?>">
											<input type="hidden" class="id_bom_detail" name="t_detail_job[<?=$i?>][id_bom_detail]" value="<?=$r->id_bom_detail?>">
											<!-- <input type="hidden" class="id_satuan" name="t_detail_job[<?=$i?>][id_satuan]" value="<?=$r->id_satuan?>" disabled> -->
											<input type="text" name="t_detail_job[<?=$i?>][qty_job]" class="form-control form-control-sm input-mask input-qty-bom" data-inputmask="'alias': 'currency', 'prefix': ''" value="<?=$r->qty_job?>"/>
										</td>
					...
					<table class="x-hidden" id="template-row">
						<tbody>
							<tr data-index="[x]">
								...
								<td>
									<input type="hidden" name="t_detail_job[x][id_detail_job]" class="id_detail_job">
									<input type="hidden" name="t_detail_job[x][id_bom_detail]" class="id_bom_detail" value="0">
									<!-- <input type="hidden" class="id_satuan" name="t_detail_job[x][id_satuan]" disabled> -->
									<input type="text" name="t_detail_job[x][qty_job]" class="form-control form-control-sm input-mask input-qty-bom" data-inputmask="'alias': 'currency', 'prefix': ''" value="0"/>
								</td>
								<td class="text-center">
									<button type="button" class="btn btn-xs btn-danger btn-delete"><i class="fa fal fa-trash"></i></button>
								</td>
