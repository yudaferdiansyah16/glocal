<main id="js-page-content" role="main" class="page-content">
	...
	<div class="row">
		<div class="col-sm-12 col-lg-12 col-xl-12">
			<div class="card mb-g">
				<form method="post" action="<?=base_url('finance/laporan_piutang/view')?>">
				...
				<div class="card-body">
					<div class="table-responsive">
						...
						<table class="table table-hover table-bordered table-striped table-sm" id="dt" role="grid">
							<thead>
							<tr>
								<th class="text-center" width="100px" rowspan="2">Kode Customer</th>
								<th class="text-center" width="150px" rowspan="2">Customer</th>
							...
							foreach($data->res->result() as $p){
								$sqlday = "SELECT DATEDIFF('".$p->tgl_bayar."','".$p->tanggal_invoice."') AS jarak";
								$resday = $this->db->query($sqlday);
								$rday = $resday->row();
								$t1 = 0;
								$t2 = 0;
								$t3 = 0;
								if($rday->jarak<=30) $t1 = $p->jumlah_bayar;
								elseif($rday->jarak>30 && $rday->jarak<=60) $t2 = $p->jumlah_bayar;
								elseif($rday->jarak>60 && $rday->jarak<=90) $t3 = $p->jumlah_bayar;
							?>
							<tr>
								<td class="text-center"><?=$p->kode_customer?></td>
								<td><?=$p->customer?></td>
								<td class="text-center"><?=$p->tanggal_invoice?></td>
								<td class="text-center"><?=$p->kode_invoice?></td>
								<td class="text-right"><?=$p->payment?></td>
								<td class="text-right"><?=number_format($p->nilai, 2)?></td>
								<td class="text-right"><?=number_format($p->jumlah_bayar, 2)?></td>
								<td class="text-right"><?=number_format($p->nilai-$p->sisa_bayar, 2)?></td>
								...
								<td class="text-right"><?=number_format($p->sisa_bayar, 2)?></td>
							</tr>
							<?php } ?>
							</tbody>
						</table>
