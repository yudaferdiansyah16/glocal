let arrdata = [];
let  oTable_t_stock_request = null;
function selectArray(arr){
	let remove = false;
	let index;
	$.each(arrdata,function(i, v){
		if(v.id_sub_barang == arr.id_sub_barang && v.id_satuan == arr.id_satuan) {
			remove = true;
			index = i;
			return false;
		}
...
$(document).ready(function(){
	let  t = 't_stock_request';
	oTable_t_stock_request = $('#dt_'+t).DataTable({
		autoWidth : true,
		responsive: false,
		scrollX: true,
		scrollY: '46vh',
		...
		info: true,
		ajax: {
			url: _baseurl+"warehouse/request_material/viewstockdt",
			type: "POST",
			// "data": function(data){
		...
		columns: [
			...
			{ data: "qty_pending", className: 'text-right', "render": function ( data, type, row, meta ) {
					return formatCurrency(data, 2) + " " + row.kode_satuan;
				} },
		],
		sorting : [[2, 'asc']],
	...
	$('#dt_'+t+'_filter input').unbind().bind('keyup', function(e) {
		if (e.keyCode === 13) {
			oTable_t_stock_request.search(this.value).draw();
		}
	});

	$('#dt_'+t+' tbody').on( 'click', 'tr', function (e) {
		let checkbox = $($(this).find('.custom-control-input')[0]);
		let data = oTable_t_stock_request.row($(this)).data();
