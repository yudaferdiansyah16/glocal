...
function selectArray(arr){
	...
	$.each(arrdata,function(i, v){
		if(v.id_stuffing === arr.id_stuffing) {
			remove = true;
			index = i;
			return false;
		}
...
$(document).ready(function(){
	let t = 't_stuffing_detail_invoice';
	let oTable_t_stuffing_detail_invoice = $('#dt_'+t).DataTable({
		autoWidth : false,
		responsive: false,
		scrollX: true,
		scrollY: '46vh',
		...
		info: true,
		ajax: {
			url: _baseurl+"sales/invoice/viewstuffingdt",
			type: "POST",
		},
		columns: [
			...
			{ data: "netto", className: 'text-right' },
			{ data: "netto", className: 'text-right' },
			{ data: "container_number", className: 'text-right' },
			{ data: "container_size", className: 'text-right' }
		],
		sorting : [[2, 'asc']],
		columnDefs: [
	...
	$('#dt_'+t+'_filter input').unbind().bind('keyup', function(e) {
		if (e.keyCode === 13) {
			oTable_t_stuffing_detail_invoice.search(this.value).draw();
		}
	});
	...
	$('#dt_'+t+' tbody').on( 'click', 'tr', function (e) {
		...
		let data = oTable_t_stuffing_detail_invoice.row($(this)).data();
	...
	$('#'+t+'_modal').on('shown.bs.modal', function () {
		setTimeout(function(){
			...
			oTable_t_stuffing_detail_invoice.columns.adjust().draw();
		},500);
	});
});
