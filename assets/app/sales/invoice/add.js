			...
			if(data.length > 0){
				for (let j = 0; j < data.length; j++) {
					...
					if(checkExist(elementTable,'.id_wh_detail', row.id_wh_detail)){
						...
						lastElement.find('.tanggal_job').text(row.tanggal_job);
						lastElement.find('.id_po').val(row.id_po);
						lastElement.find('.id_stuffing').val(row.id_stuffing);
						lastElement.find('.id_satuan').val(row.id_satuan);
						lastElement.find('.id_kemasan').val(row.id_kemasan);
						lastElement.find('.tanggal_job').text((row.tanggal_job != null ? moment(row.tanggal_job).format('DD-MM-YYYY') : '-'));
						...
						lastElement.find('.kode_satuan').text(row.kode_satuan);
						lastElement.find('.qty_invoice').val(row.qty_invoice);
						lastElement.find('.qty_remain').text(row.qty_remain);
						lastElement.find('.po_buyer').text(row.po_buyer);
						lastElement.find('.tanggal_stuffing').text(row.tanggal_stuffing);
						lastElement.find('.tanggal_stuffing').text(row.tanggal_stuffing);
					}
