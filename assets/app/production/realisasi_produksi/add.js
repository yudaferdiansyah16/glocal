	...
	switch(key) {
		case 't_production_detail_realisasi_material':
			...
			for (let j = 0; j < data.length; j++) {
				...
				lastElementMaterial.find('.text_qty_request_material').text(row.qty * (row.unit_konversi == null ? 1 : row.unit_konversi));
		...
		case 't_production_detail_realisasi_wip':
			...
			for (let j = 0; j < data.length; j++) {
				...
				if(row.qty_max) lastElementWip.find('.qty_wip').attr('max',row.qty_max * (row.unit_konversi == null ? 1 : row.unit_konversi));
		...
		case 't_production_detail_realisasi_scrap':
			...
			for (let j = 0; j < data.length; j++) {
				...
				if(row.qty_max) lastElementScrap.find('.qty_scrap').attr('max',row.qty_max * (row.unit_konversi == null ? 1 : row.unit_konversi));
		...
		case 't_production_detail_realisasi_waste':
			...
			for (let j = 0; j < data.length; j++) {
				...
				if(row.qty_max) lastElementWaste.find('.qty_waste').attr('max',row.qty_max * (row.unit_konversi == null ? 1 : row.unit_konversi));
		...
		case 't_production_detail_realisasi_loss':
			...
			for (let j = 0; j < data.length; j++) {
				...
				if(row.qty_max) lastElementLoss.find('.qty_loss').attr('max',row.qty_max * (row.unit_konversi == null ? 1 : row.unit_konversi));
	...
	let oTableFg = $('#dt_fg').DataTable({
		...
		"columns": [
			...
			{  render: function (data, type, row) {
				// $a=data.id_detail_po;
				return '<input class="form-control" id="qty_realisasi" name="t_detail_dn[qty_realisasi]" type="text"  value=""  /><input class="form-id_detail_po" id="id_detail_po" name="t_detail_dn[id_detail_po]" type="hidden"  value="' + row.id_detail_po + '"  />';
			}},
		],
		"sorting" : [],
		"paginate": false,
