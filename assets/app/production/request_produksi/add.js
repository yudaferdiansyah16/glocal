	...
	switch(key) {
		case 't_wh_detail_request':
			...
			for (let j = 0; j < data.length; j++) {
				...
				let unit_konversi = row.unit_konversi == null ? 1 : row.unit_konversi;
				...
				lastElement.find('.text_uraian_satuan').text(row.uraian_satuan_terkecil);
				...
				lastElement.find('.text_qty_wh').text(formatQuantity(Number(row.qty_sisa * unit_konversi)));
				lastElement.find('.qty').attr('max',row.qty_sisa * unit_konversi);
				...
				lastElement.find('.id_detail_dna').val(row.id_detail_dn);
