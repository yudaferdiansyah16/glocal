let elementTable = $('#dt');
let deleted_detail_dn = [];

function setValue(column, data){
	let index = '-1';
	switch (column) {
		case 'm_supplier':
			$('.id_supplier').val(data.id_suplier);
...
$(document).ready(function(){
	...
	$(document).on('click', '.btn-delete', function () {
		...
		const id_detail_dn = tr.find('.id_detail_dn').val();
		deleted_detail_dn.push(id_detail_dn);
		$('#deleted_detail_dn').val(JSON.stringify(deleted_detail_dn));
		tr.remove();
		renderTableNumber(elementTable, 0);
	});
	$(".input-mask").inputmask({removeMaskOnSubmit: true});
