$(document).ready(function(){
	let oTable = $('#dt').DataTable({
		...
		"info": true,
		"ajax": {
			url: _baseurl+"warehouse/issue_material/viewdt",
			type: "POST",
		},
		"columns": [
			{ data: "no", searchable: false, className: 'text-center' },
			{ data: "kode_mutasi", render: function ( data, type, row, meta ) {
					return data + "<br><small>Issue Date: " + moment(row.tanggal_terima).format('DD-MM-YYYY') + "</small><br>" + "<span class='badge badge-info'>"+row.nama_jenis_mutasi+"</span>";
			} },
			{ data: "tanggal_terima", className: "text-center", render: renderDTDate, visible: false },
